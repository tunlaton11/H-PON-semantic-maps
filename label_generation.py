...
def process_sample_data(nuscenes, map_data, sample_data, lidar, config):
    ...
    masks[-1] |= ~nusc_utils.get_visible_mask(intrinsics, sample_data['width'], 
    ...
    masks[-1] |= nusc_utils.get_occlusion_mask(cam_points, config.map_extents,
                                    config.map_resolution)
...
if __name__ == '__main__':
    ...
    dataroot = os.path.join(os.getcwd(), config.data_root)
    nuscenes = NuScenes(config.version, dataroot)
